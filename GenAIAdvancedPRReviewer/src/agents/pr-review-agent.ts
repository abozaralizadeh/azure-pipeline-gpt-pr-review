...
export class AdvancedPRReviewAgent {
  ...
  constructor(
    ...
    // Initialize Azure OpenAI client with basic configuration
    this.model = new ChatOpenAI({
      openAIApiKey: azureOpenAIKey,
      temperature: 0.1,
      maxTokens: 4000
    });
  }
  ...
  private async analyzeContext(state: PRReviewStateType): Promise<PRReviewStateType> {
    ...
    const contextPrompt = `Analyze the Pull Request context and determine if a detailed review is necessary.
...
Target Branch: ${state.pr_context?.target_branch || "N/A"}
Source Branch: ${state.pr_context?.source_branch || "N/A"}

Based on this information, determine if the PR requires a detailed code review. Consider:
1. Is this a meaningful change that affects functionality?
2. Are there security implications?
3. Does this change require testing?
4. Is the scope appropriate for a PR?
...
Respond with JSON:
{
  ...
  "overall_security_score": "A" | "B" | "C" | "D" | "F"
    ...
    try {
      const response = await this.model.invoke([
        new HumanMessage(securityPrompt)
      ]);

      const securityAnalysis = JSON.parse(response.content as string);
...
Provide a final recommendation in JSON format:
{
  ...
  "confidence": number (0.0-1.0)
    ...
    try {
      ...
      const finalAssessment = JSON.parse(response.content as string);
